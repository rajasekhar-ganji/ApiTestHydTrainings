...
public class GetByIdRequestWireMockwithNTLMAuth {
	...
	public void get() {
		...
		response = RestAssured.given().baseUri(Constant.baseurl).header(Constant.content, Constant.contenttype).auth()
				.ntlm(Constant.ntlmusername, Constant.ntlmpassword, Constant.ntlmdomain, Constant.ntlmworkstation)
				.accept(ContentType.JSON).when().get(Constant.getbyid).then().assertThat()
				.statusCode(Constant.statuscode).and().log().all().extract().response();
