...
public class GetRequestWireMockwithBasicAuth {
	...
	public void get() throws URISyntaxException {
		Response response = RestAssured.given().baseUri(Constant.baseurl).header(Constant.content, Constant.contenttype)
				.auth().basic(Constant.basicUsername, Constant.basicPassword).accept(ContentType.JSON).when()
				.get(Constant.get).then().assertThat().statusCode(Constant.statuscode).and().log().all().extract()
				.response();
