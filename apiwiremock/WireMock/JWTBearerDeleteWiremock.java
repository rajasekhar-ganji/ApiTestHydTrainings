...
public class JWTBearerDeleteWiremock {
	...
	public void delete() throws URISyntaxException {
		Response response = RestAssured.given().baseUri(Constant.baseurl).header(Constant.content, Constant.contenttype)
				.header(Constant.authorization, Constant.bearer + Constant.JwtBearerToken).accept(ContentType.JSON)
				.when().delete(Constant.delete).then().assertThat().statusCode(Constant.statuscode).log().all()
				.extract().response();
